...
def run_retrieve_evaluation(relevance, predictions):
    scores = {}

    for compression, prediction in predictions.items():
...
def run_retrieve_task(
    ...
    if dataset_name == "bm25":
        ...
        if args.load_embds:
            ...
            if hasattr(encoder, 'local_existing_ids') and len(
                    encoder.local_existing_ids) == encoder.expected_dataset_size:
                trainer.print(
                    f"All {len(encoder.local_existing_ids)} query embeddings already exist, skipping encoding")
                need_encoding = False
                ...
                trainer.print(
                    f"Resuming query encoding: {len(encoder.local_existing_ids)}/{encoder.expected_dataset_size} exist, generating {missing_count} missing embeddings")
        ...
        if args.load_embds:
            ...
            if hasattr(encoder, 'local_existing_ids') and len(
                    encoder.local_existing_ids) == encoder.expected_dataset_size:
                trainer.print(
                    f"All {len(encoder.local_existing_ids)} corpus embeddings already exist, skipping encoding")
                need_encoding = False
            elif hasattr(encoder, 'local_existing_ids') and len(encoder.local_existing_ids) > 0:
                missing_count = encoder.expected_dataset_size - len(encoder.local_existing_ids)
                trainer.print(
                    f"Resuming corpus encoding: {len(encoder.local_existing_ids)}/{encoder.expected_dataset_size} exist, generating {missing_count} missing embeddings")
